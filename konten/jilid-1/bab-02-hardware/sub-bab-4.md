...
Untuk memahami perbedaan generasi SSD, kita harus mengenal dulu jalan raya yang dilaluinya: **PCIe** (*Peripheral Component Interconnect Express*). PCIe bekerja dengan *lane* — lajur data pararel — dan setiap generasi menggandakan kecepatan setiap lajurnya. SSD NVMe konsumen umumnya menggunakan 4 lane (ditulis x4), sehingga kecepatan puncaknya adalah hasil kali laju per lane dengan jumlah lane.
...
Bagi pengguna dengan RAM terbatas, ada satu teknik riset lagi yang layak dikenal: **windowing dan row-column bundling** dari paper *LLM in a Flash* [5]. Penelitian ini menunjukkan bahwa model hingga 2x ukuran DRAM masih bisa diinferensikan dengan andal dari flash storage — selama weights dipetakan ulang secara cerdas (mengelompokkan baris dan kolom agar blok yang dibutuhkan berdekatan) dan bagian yang jarang dipakai ditinggalkan di disk [5]. Implikasinya bagi pengguna rumahan dengan RAM 16-32 GB: model 32-64 GB bukanlah batas mutlak, melainkan sebuah teka-teki tata letak data — dan kombinasi windowning + SSD cepat bisa menjadi cara bertahan sebelum upgrade RAM.
...
## 8. Tabel Perbandingan Storage

### Tabel 1: Spesifikasi NVMe SSD untuk AI Workstation
...
Panduan praktisnya sederhana. Jika anggaran Anda hanya cukup untuk satu SSD: Gen 4 2 TB (SN850X sekitar Rp 2,5 juta atau 990 Pro sekitar Rp 3,0 juta — spesifikasi resmi 990 Pro tersedia di halaman Samsung [9]) adalah pilihan paling seimbang — kapasitas besar untuk menampung model dan dataset, dengan kecepatan yang sudah dua kali lipat Gen 3. Jika beban kerja Anda didominasi model 70B ke atas, Gen 5 (Micron 4600 1 TB sekitar Rp 3,5 juta) lebih menarik meskipun kapasitasnya lebih kecil — setengah detik menuju 70B terasa jauh lebih menyenangkan daripada kapasitas ekstra yang jarang terpakai. Dan bila motherboard Anda masih PCIe Gen 3, tahan diri: beli SSD Gen 4 yang murah namun kompatibel mundur, lalu alokasikan sisa anggaran untuk upgrade platform — karena di slot Gen 3, premium Gen 5 akan sia-sia.

Bagi yang menargetkan model raksasa, satu hubungan langsung perlu dicatat: ukuran file model ditentukan oleh jumlah parameter dan kuantisasinya — DeepSeek V4 Flash (284B) dalam Q4_K_M sekitar 160 GB dan Mistral Large 3 (675B) Q3 sekitar 280 GB, sementara DeepSeek V4 Pro (1,6T) jauh lebih besar lagi [10]. Semakin besar file, semakin besar peran *sequential read* — dan semakin tegas rekomendasi Gen 4/5. Ringkasan yang adil dari seluruh seksi ini: **disk adalah gudang model Anda; gudang yang sempit membuat seluruh rumah menunggu.**

### Tabel 2: Dampak Storage pada TTFT Cold Start
...
## 10. Praktikum: Mengukur dan Mengoptimalkan Storage Anda
...
## 11. Studi Kasus: Upgrade SSD Gen 3 ke Gen 5 untuk Multi-Model Workflow
...
**Sesudah upgrade.** Ia mengganti ke Crucial T700 (Gen 5, 12.4 GB/s) seharga sekitar Rp 4,5 juta untuk varian 2 TB. Loading 7B turun menjadi 0,10 detik dan 14B menjadi 0,18 detik — hampir 4x lebih cepat. Dampaknya sangat konkret: saat berganti model 20x sehari, ia menghemat sekitar 15 detik per hari, yang terakumulasi menjadi sekitar 5 jam per tahun — hampir satu hari kerja yang kembali.

**Analisis biaya.** Crucial T700 2 TB sekitar Rp 4,5 juta berbanding Samsung 970 EVO Plus 1 TB sekitar Rp 1,2 juta. Selisih Rp 3,3 juta untuk menghemat 5 jam setahun mungkin terdengar mahal — tetapi bagi profesional yang dibayar per jam, angka ini lunas dalam hitungan minggu. Lebih penting lagi, efeknya juga terasa saat menjalankan model 70B (1,5 detik berbanding 3,0 detik) dan saat membuka DeepSeek V4 Flash (160 GB) di masa depan.
...
[7] NVIDIA & UIUC. *BaM: GPU-Initiated Storage Access*. [https://github.com/ZaidQureshi/bam](https://github.com/ZaidQureshi/bam)

[8] TechPowerUp. *SSD Benchmarks*. [https://www.techpowerup.com/review/ssd](https://www.techpowerup.com/review/ssd)
